#!/usr/bin/env python3
"""
Main execution script for CGM Federated Learning Benchmark V1
...
# Add src_v1 to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src_v1'))
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src_v1'))
...
def main():
    ...
    print("=" * 60)
    print("CGM Federated Learning Benchmark V1")
    print("=" * 60)
    ...
    try:
        # Load configurations (V1)
        configs_dir = os.path.join(os.path.dirname(__file__), '..', 'configs_v1')
        with open(os.path.join(configs_dir, 'data_config.json'), 'r') as f:
            data_config_dict = json.load(f)
        
        with open(os.path.join(configs_dir, 'experiment_config.json'), 'r') as f:
        ...
        print(f"📊 Results saved in: experiments_v1/{experiment_config.experiment_name}/")
